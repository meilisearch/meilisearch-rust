...
mod tests {
    ...
    fn minimal_stats() -> serde_json::Value {
        serde_json::json!({
            "totalNbTasks": 0,
            "status": {},
            "types": {},
    ...
    #[tokio::test]
    async fn test_batch_stats_progress_trace() {
        let mut s = mockito::Server::new_async().await;
        let base = s.url();
        ...
        let client = Client::new(base, None::<String>).unwrap();
        let batch = client.get_batch(1).await.expect("get batch failed");

        let trace = &batch.stats.progress_trace;
        assert_eq!(trace.get("indexing").map(String::as_str), Some("42ms"));
        assert_eq!(trace.get("encoding").map(String::as_str), Some("10ms"));
    }

    #[tokio::test]
    async fn test_batch_stats_write_channel_congestion() {
        let mut s = mockito::Server::new_async().await;
        let base = s.url();
        ...
        let client = Client::new(base, None::<String>).unwrap();
        let batch = client.get_batch(2).await.expect("get batch failed");

        let congestion = &batch.stats.write_channel_congestion;
        assert_eq!(congestion.get("pending").map(String::as_str), Some("3"));
        assert_eq!(congestion.get("capacity").map(String::as_str), Some("100"));
    }

    #[tokio::test]
    async fn test_batch_stats_internal_database_sizes() {
        let mut s = mockito::Server::new_async().await;
        let base = s.url();
        ...
        let client = Client::new(base, None::<String>).unwrap();
        let batch = client.get_batch(3).await.expect("get batch failed");

        let db_sizes = &batch.stats.internal_database_sizes;
        assert_eq!(db_sizes.get("main").map(String::as_str), Some("1048576"));
        assert_eq!(
            db_sizes.get("documents").map(String::as_str),
            Some("524288")
        );
    }

    #[tokio::test]
    async fn test_batch_stats_all_new_fields_present_in_list() {
        let mut s = mockito::Server::new_async().await;
        let base = s.url();
            ...
            "results": [
                {
                    ...
                    "stats": {
                        ...
                        "indexedUids": { "books": 3 },
                        "progressTrace": { "merging": "5ms" },
                        "writeChannelCongestion": { "pending": "0" },
                        "internalDatabaseSizes": { "main": "2097152" }
                    }
                }
            ],
            "limit": 20,
        ...
        let stats = &batches.results[0].stats;
        assert_eq!(
            stats.progress_trace.get("merging").map(String::as_str),
            Some("5ms")
        );
        assert_eq!(
            stats
                .write_channel_congestion
                .get("pending")
                .map(String::as_str),
            Some("0")
        );
